...
public class StatisticService {
    ...
    public String export(String filePath, File file, HttpServletResponse response, String showSource, String sourceReplace, String showCompetitorUrl) throws IOException {
        ...
        List<List<Object>> resultTest = getResultList2(originalWb, newColumn, sourceReplace);
        List<StatisticDTO> resultDTO = getDTOList(result, showSource, sourceReplace, showCompetitorUrl);
    ...
    private static List<List<Object>> getResultList2(List<List<Object>> list, List<Integer> columnList, String sourceReplace) {
        List<List<Object>> newList = new LinkedList<>();
        int counter = 0;
        for (int i = 0; counter < list.size(); i++) {
            counter++;
            List<Object> linked = new LinkedList<>();
            getRowList2(linked, columnList, list.get(i), sourceReplace);
            newList.add(linked);
        }
        return newList;
    }

    private static void getRowList2(List<Object> linked, List<Integer> columnList, List<Object> row, String sourceReplace) {
        Object value;
